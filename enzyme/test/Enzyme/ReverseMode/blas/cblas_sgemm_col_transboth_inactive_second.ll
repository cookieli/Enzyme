...
;CHECK:define internal { float, float } @diffeg(float* noalias %A, float* %"A'", float* %C, float* %"C'", float %alpha, float %beta) {
;CHECK-NEXT:entry:
;CHECK-NEXT:  %B = alloca [6 x float], align 16
;CHECK-NEXT:  %0 = bitcast [6 x float]* %B to i8*
;CHECK-NEXT:  call void @llvm.memcpy.p0i8.p0i8.i64(i8* align 16 %0, i8* align 16 bitcast ([6 x float]* @__const.g.B to i8*), i64 24, i1 false)
;CHECK-NEXT:  %arraydecay = getelementptr inbounds [6 x float], [6 x float]* %B, i32 0, i32 0
;CHECK-NEXT:  call void @cblas_sgemm(i32 102, i32 112, i32 112, i32 4, i32 3, i32 2, float %alpha, float* nocapture readonly %A, i32 2, float* nocapture readonly %arraydecay, i32 3, float %beta, float* %C, i32 4)
;CHECK-NEXT:  call void @cblas_sgemm(i32 102, i32 111, i32 111, i32 4, i32 2, i32 3, float %alpha, float* nocapture readonly %"C'", i32 4, float* nocapture readonly %arraydecay, i32 3, float 1.000000e+00, float* %"A'", i32 4)
